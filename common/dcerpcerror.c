...
{
    return dwErrorCode >= rpc_s_mod &&
        dwErrorCode <= rpc_s_fault_codeset_conv_error;
}

uint32_t
PMDGetDceRpcShortErrorString(
    uint32_t dwRpcError,
    char** ppszErrorMessage)
{
    ...
    dwError = PMDAllocateString(s, &pszError);
    BAIL_ON_PMD_ERROR(dwError);

    *ppszErrorMessage = pszError;
cleanup:
    return dwError;
...
{
    char* s = "unknown rpc error occured, error = %s (0x%08x).";
    char* pszError = NULL;
    char* pszShortError = NULL;
    uint32_t dwError = 0;
    switch (dwRpcError)
    {
    case rpc_s_mod                       :
    ...
    dwError = PMDGetDceRpcShortErrorString(dwRpcError, &pszShortError);
    BAIL_ON_PMD_ERROR(dwError);
    dwError = PMDAllocateStringPrintf(&pszError, s, pszShortError, dwRpcError);
    BAIL_ON_PMD_ERROR(dwError);

    *ppszErrorMessage = pszError;
cleanup:
    PMD_SAFE_FREE_MEMORY(pszShortError);
    return dwError;
