...
pmd_policy_plugin_load_interface(PPMD_POLICY_PLUGIN_INTERFACE *ppInterface)
{
    ...
    gpServerEnv->gpGroupInterface->handle = dlopen(gp_plugin_paths[0],RTLD_NOW);
    if(!gpServerEnv->gpGroupInterface->handle)
    {
    ...
    gpServerEnv->gpGroupInterface->pFnPolicyVersion =dlsym(gpServerEnv->gpGroupInterface->handle,"pmd_gpmgmt_get_version");
    ...
    )
{
    ...
    if(gpServerEnv->gpGroupInterface->handle)
    {
    dwError = dlclose(gpServerEnv->gpGroupInterface->handle);
    if(dwError !=0)
      {
