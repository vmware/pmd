...
Operating System Home URL: https://vmware.github.io/photon/
     ...
     Network Online State: online
                  ...
                  Address: 172.16.130.132/24 on link ens33
                           ...
                           172.16.130.138/24 on link test99
                  Gateway: 172.16.130.2 on link test99
...
>pmctl network add-sriov dev test99 vf 2 vlanid 1 qos 1024 vlanproto 802.1Q macsfc yes qrss yes trust yes linkstate auto macaddr 00:0c:29:3a:bc:11
...
>pmctl network add-dns dev test99 dns 8.8.8.8,8.8.4.4,8.8.8.1,8.8.8.2

#Configure network domains
pmctl network add-domain dev <deviceName> domains <domainlist>
>pmctl network add-domain dev test99 domains test1.com,test2.com,test3.com,test4.com

#Configure network ntp
pmctl network add-ntp dev <deviceName> ntp <ntplist>
>pmctl network add-ntp dev test99 ntp 198.162.1.15,test3.com

# Configure network ipv6AcceptRA
pmctl network set-ipv6-accept-ra <deviceName> <IPv6AcceptRA>
>pmctl network set-ipv6-accept-ra test99 false
...
>pmctl network add-rule dev test99 tos 12 from 192.168.1.10/24 to 192.168.2.20/24 fwmark 7/255 table 8 prio 3 iif test99 oif test99 srcport 8000-8080 destport 9876 ipproto 17 invertrule yes family ipv4 usr 1001 suppressprefixlen 128 suppressifgrp 2098 type prohibit
...
>pmctl network delete-rule dev test99 tos 12 from 192.168.1.10/24 to 192.168.2.20/24 fwmark 7/255 table 8 prio 3 iif test99 oif test99 srcport 8000-8080 destport 9876 ipproto 17 invertrule yes family ipv4 usr 1001 suppressprefixlen 128 suppressifgrp 2098 type prohibit
...
>pmctl network set-dhcpv4-id dev test99 clientid duid vendorclassid 101 iaid 201

# Configure network DHCPv4 duid
pmctl network set-dhcpv4-duid dev <deviceName> duidtype <DUIDType> duidrawdata <DUIDRawData>
>pmctl network set-dhcpv4-duid dev test99 duidtype vendor duidrawdata af:03:ff:87
...
>pmctl network set-dhcpv4-use dev test99 usedns false usentp false usesip false usemtu yes usehostname true usedomains yes useroutes no usegateway yes usetimezone no
...
>pmctl network set-dhcpv6 dev test99 mudurl https://example.com/devB userclass usrcls1,usrcls2 vendorclass vdrcls1 prefixhint 2001:db1:fff::/64 withoutra solicit
...
>pmctl network set-dhcpv6-id dev test99 iaid 201 duidtype vendor duidrawdata af:03:ff:87
...
>pmctl network set-dhcpv6-use dev test99 useaddr yes useprefix no usedns false usentp false usehostname true usedomains yes
...
>pmctl network set-dhcpv6-option dev test99 reqopt 10,198,34 sendopt 34563 sendvendoropt 1987653,65,ipv6address,af:03:ff:87
...
>pmctl network add-dhcpv4-server dev test99 pool-offset 100 pool-size 200 default-lease-time-sec 10 max-lease-time-sec 30 dns 192.168.1.2,192.168.10.10,192.168.20.30 emit-dns yes emit-ntp no emit-router yes

# Remove network DHCPServer
pmctl network remove-dhcpv4-server <Devicename>
>pmctl network remove-dhcpv4-server test99
...
>pmctl network add-ipv6ra dev test99 rt-pref medium emit-dns yes dns 2002:da8:1::1,2002:da8:2::1 emit-domains yes domains test1.com,test2.com dns-lifetime-sec 100 prefix 2002:da8:1::/64 pref-lifetime-sec 100 valid-lifetime-sec 200 assign yes route 2001:db1:fff::/64 lifetime-sec 1000

# Remove network IPv6SendRA
pmctl network remove-ipv6ra <Devicename>
>pmctl network remove-ipv6ra test99
...
>pmctl network create-vlan vlan1 dev test99 id 101
...
>pmctl network create-bond bond1 dev test99,test98 mode 802.3ad thp layer2+3 ltr slow mms 1s

# Configure Bond with default
>pmctl network create-bond bond1 dev test99,test98

# Configure Bridge with default
pmctl network create-bridge <bridgeName> dev <device list>
>pmctl network create-bridge br0 dev test99,test98

# Configure MacVLan
pmctl network create-macvlan <macvlanName> dev <device> mode <modeName>
>pmctl network create-macvlan macvlan1 dev test99 mode private

# Configure IpVLan
pmctl network create-ipvlan <ipvlanName> dev <device> mode <modeName> flags <flagsName>
>pmctl network create-ipvlan ipvlan1 dev test99 mode l2 flags vepa

# Configure IpVLan with default
>pmctl network create-ipvlan ipvlan1 dev test98
...
>pmctl network create-vxlan vxlan1 dev test99 vni 16777215 remote 192.168.1.3 local 192.168.1.2 group 192.168.0.0 destport 4789 independent no
...
>pmctl network create-wg wg1 dev test99 skey wCmc/74PQpRoxTgqGircVFtdArZFUFIiOoyQY8kVgmI= pkey dSanSzExlryduCwNnAFt+rzpI5fKeHuJx1xx2zxEG2Q= endpoint 10.217.69.88:51820 port 51822 ips fd31:bf08:57cb::/48,192.168.26.0/24

# Configure WireGuard with default
>pmctl network create-wg wg1 dev test99 skey wCmc/74PQpRoxTgqGircVFtdArZFUFIiOoyQY8kVgmI= pkey dSanSzExlryduCwNnAFt+rzpI5fKeHuJx1xx2zxEG2Q= endpoint 10.217.69.88:51820
```
# Configure Tun
...
>pmctl network create-tun tun99 dev test99 mq yes pktinfo yes vnet-hdr no usr test-user grp test-group kc no
```
...
>pmctl network create-tun tap99 dev test99 mq yes pktinfo yes vnet-hdr no usr test-user grp test-group kc no
```
 
#### Remove network device using pmctl
```bash
pmctl network remove-netdev <kindDeviceName> kind <kindType>
>pmctl network remove-netdev ipvlan1 dev test99 kind ipvlan
```
...
>pmctl link set-name dev test99 namepolicy mac,kernel,database,onboard,keep,slot,path
...
>pmctl link set-alt-name dev test99 altnamespolicy mac,database,onboard,slot,path
...
>pmctl link set-csum-offload dev test99 rxco true txco true
...
>pmctl link set-tcp-offload dev test99 tcpso true tcp6so true
...
>pmctl link set-generic-offload dev test99 gso true gro true grohw false gsomaxbytes 65536 gsomaxseg 65535
...
>pmctl link set-vlan-tags dev test99 rxvlanctaghwacl true txvlanctaghwacl false rxvlanctagfilter true txvlanstaghwacl true
...
>pmctl link set-channel dev test99 rxch 1024 txch 2045 och 45678 coch 32456
...
>pmctl link set-buffer dev test99 rxbufsz 100009 rxmbufsz 1998 rxjbufsz 10999888 txbufsz 83724

# Configure Link Queues.
pmctl link set-queue dev <deviceName> rxq <ReceiveQueues> txq <TransmitQueues> txqlen <TransmitQueueLength>
>pmctl link set-queue dev test99 rxq 4096 txq 4096 txqlen 4294967294
...
>pmctl link set-flow-ctrl dev test99 rxfctrl true txfctrl true anfctrl true
...
>pmctl link set-adpt-coalesce dev test99 uarxc true uatxc true
...
>pmctl link set-rx-coalesce dev test99 rxcs 23 rxcsirq 56 rxcslow 5 rxcshigh 76788
...
>pmctl link set-tx-coalesce dev test99 txcs 23 txcsirq 56 txcslow 5 txcshigh 76788
...
>pmctl link set-rx-coald-frames dev test99 rxmcf 23 rxmcfirq 56 rxmcflow 5 rxmcfhigh 76788
...
>pmctl link set-tx-coald-frames dev test99 txmcf 23 txmcfirq 56 txmcflow 5 txmcfhigh 76788
...
>pmctl link set-coalesce-pkt dev test99 cprlow 1000 cprhigh 32456 cprsis 102

# Configure Link Alias,Description,port,duplex...etc.
pmctl link set-link dev test99 alias <Alias> desc <Description> mtub <MTUBytes> bits <BitsPerSecond> duplex <Duplex> auton <AutoNegotiation> wol <WakeOnLan> wolpassd <WakeOnLanPassword> port <Port> advertise <Advertise> lrxo <LargeReceiveOffload> ntf <NTupleFilter> ssbcs <StatisticsBlockCoalesceSec>
>pmctl link set-link dev test99 alias ifalias desc configdevice mtub 10M bits 5G duplex full auton no wol phy,unicast,broadcast,multicast,arp,magic,secureon wolpassd cb:a9:87:65:43:21  port mii advertise 10baset-half,10baset-full,20000basemld2-full lrxo true ntf true ssbcs 1024
...
>pmctl status proc net path ipv6 dev test99 property mtu
                 Path: ipv6
                 Link: test99
             Property: mtu
                Value: 1300
...
>pmctl proc net path ipv6 dev test99 property mtu value 1500
